	...
	public static void init() {
		...
		if (!Help.get("clear").isPresent()) {
			Usage usage = new Usage()
					.addArgument(new Argument("[player]", "Must be a player name or a target selector. If not specified, defaults to the player who executed the command. Not optional in command blocks."))
					.addArgument(new Argument("[item]", "Specifies the id of the item to be cleared. If not specified, all items in the player's inventory are cleared."))
					.addArgument(new Argument("[data]", "Specifies the data value of the item to be cleared. If not specified, or if -1, all items that match item are cleared, regardless of their data."))
					.addArgument(new Argument("[maxCount]", "Specifies the maximum number of items to be cleared. If not specified, or if -1, all items that match item and data are cleared. If 0, does not clear items, but returns successfully if there were items that could have been cleared"))
					.addArgument(new Argument("[dataTag]", "Specifies the data tags of the item to be cleared (only items matching the specified data tags will be cleared. Data tags not specified will not be considered). Must be a compound NBT tag (for example, {display:{Name:Fred}})."));
		...
		if (!Help.get("give").isPresent()) {
			Usage usage = new Usage()
					.addArgument(new Argument("<player>", "Specifies the target to give item(s) to. Must be a player name or target selector."))
					.addArgument(new Argument("<item>", "Specifies the item to give. Must be a valid item id (for example, minecraft:iron_shovel), or block id for which items exist. Numerical ids are unsupported."))
					.addArgument(new Argument("[amount]", "Specifies the number of items to give. Must be between 1 and 64 (inclusive), but can be 64 even when that's more than one stack. If not specified, defaults to 1."))
		...
		if (!Help.get("teleport").isPresent()) {
			Usage usage = new Usage()
					.addArgument(new Argument("<target entity>", "Specifies the entity(s) to be teleported. Must be either a player name or a target selector."))
					.addArgument(new Argument("<x> <y> <z>", "Specifies the coordinates to teleport the target(s) to. x and z must fall within the range -30,000,000 to 30,000,000 (exclusive, without the commas), and y must be within the range -4096 to 4096 inclusive. May use tilde notation to specify a position relative to the position of the executor of the command (use /tp or /execute to teleport using coordinates relative to the target instead)."))
